...
class TestReconstitution:
    ...
    def test_initial_members_returned_with_no_changes(self) -> None:
        ...
        members, diag = get_universe_at_date(
            changes, target_date=date(2024, 6, 1), initial_members=["AAPL", "MSFT"]
        )
        assert members == ["AAPL", "MSFT"]

    def test_add_applies(self) -> None:
        changes = _make_changes([("2024-03-01", "GOOG", "ADD")])
        members, _ = get_universe_at_date(changes, target_date=date(2024, 6, 1), initial_members=["AAPL"])
        assert "GOOG" in members
        assert "AAPL" in members

    def test_remove_applies(self) -> None:
        changes = _make_changes([("2024-03-01", "AAPL", "REMOVE")])
    ...
    def test_output_is_sorted(self) -> None:
        changes = _make_changes([])
        members, _ = get_universe_at_date(
            changes, target_date=date(2024, 6, 1), initial_members=["MSFT", "AAPL", "GOOG"]
        )
...
class TestPointInTime:
    ...
    def test_future_add_excluded(self) -> None:
        changes = _make_changes([("2025-01-01", "NVDA", "ADD")])
        members, diag = get_universe_at_date(changes, target_date=date(2024, 6, 1), initial_members=["AAPL"])
...
class TestDiagnostics:
    ...
    def test_missing_columns_produces_error(self) -> None:
        ...
        _, diag = get_universe_at_date(bad_df, date(2024, 6, 1), ["AAPL"])
