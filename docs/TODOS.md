...
### TODO-36: Stabilize Quintile Construction Tie-Breaking for Discrete-Score Factors
**What:** In `src/nyse_core/factor_screening.py::compute_long_short_returns`, introduce a deterministic secondary sort by `symbol` before `pd.qcut`/rank quintile assignment so that discrete-score factors (e.g., piotroski F-score ∈ {0..9}) reproduce bit-exactly across runs regardless of DuckDB query-plan row order.
**Why:** iter-0 (2026-04-21) re-screened all six Tier-1/Tier-2 factors. Five (ivol_20d, high_52w, momentum_2_12, accruals, profitability) reproduced bit-exactly vs the 2026-04-18 ledger. **Piotroski G0 OOS Sharpe drifted 0.0385 → 0.0181 (-53% relative)** while its G2 ic_mean (0.0090) and G3 ic_ir (0.089) held essentially stable across runs. Root cause: piotroski is a discrete integer in [0, 9], producing many tied values per rebalance date; `pd.qcut` resolves ties using the input Series' positional order, which inherits DuckDB's non-deterministic query-plan order, which shifts quintile membership for tied observations, which moves the long/short Sharpe non-trivially. Continuous-score factors have no ties so they reproduce exactly. The IC is invariant to tie ordering because Spearman correlation averages ranks within ties, which explains the G2/G3 stability.
**Why this is AP-6 sensitive:** Any fix changes the computed G0 Sharpe on the Apr-18 factor set. Per AP-6 ("no post-hoc threshold or methodology changes after observing results"), the fix MUST be pre-registered as a `correction` event in `results/research_log.jsonl` BEFORE re-screening piotroski; the correction event must record (1) the specific change (deterministic secondary sort key = `symbol`), (2) why the prior behavior was a defect rather than a methodology choice (DuckDB row-order is implementation-defined, not a legitimate tie-breaker), (3) that both piotroski Sharpe values were FAIL against the G0 ≥ 0.30 floor so the admission verdict is unchanged regardless. Post-fix re-screen of ALL SIX factors to confirm 6/6 bit-exactness.
**How to apply:** (1) Add `.sort_values(["date", "symbol"])` (or equivalent groupby-sort) to `compute_long_short_returns` immediately before the per-date `qcut`/quintile assignment. (2) Add a deterministic tie-breaker unit test in `tests/unit/test_factor_screening.py` that constructs a synthetic factor with >50% ties and asserts identical long/short Sharpe across two runs whose input DataFrames differ only by row-order shuffle. (3) Run the correction pre-registration flow per AP-6: write a `correction_preregistration` event BEFORE touching the code, then a `correction_applied` event AFTER the code change with re-screen results. (4) Re-screen all six factors. Expected outcome: all six reproduce bit-exactly post-fix (piotroski's new Sharpe becomes the canonical one and is recorded as the reference going forward).
**Slot in:** Wave 3 iter-10 (gate calibration pre-registration) is the natural home — the correction can be bundled into the same pre-registration document so Wave 3 ratification ratifies both the tie-breaker fix and the new Tier-2 thresholds in one atomic step.
**Depends on:** TODO-33 (canonical-append enforcement, helpful but not blocking). No external dependencies.
