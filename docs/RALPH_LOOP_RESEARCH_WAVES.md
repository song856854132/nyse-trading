...
**Wave 3 (C-gate-calibration) HALTED PENDING OPERATOR AUTHORIZATION.** iter-9 shipped the audit memo (commit `ead47d8`, findings only). iter-10 presented the findings as a research-log event and halts here. iter-11 (correction-path application) is a **one-way door — requires explicit operator `correction` authorization event naming the chosen path (A / B / C / D) before proceeding**. The loop will NOT advance to Wave 4 until operator resolves iter-11.

**Current research-log chain tip:** `677f39bf37926e4f540d5577ccebf297e3a96125ab5c88eef2dc09af97f814ab` (iter-10, 2026-04-23).
