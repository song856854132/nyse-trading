...
**Wave 3 (C-gate-calibration) HALTED PENDING OPERATOR AUTHORIZATION.** iter-9 shipped the audit memo (commit `ead47d8`, findings only). iter-10 presented the findings as a research-log event and halts here (commit `63821f4`). iter-10 supplemental shipped `docs/audit/gate_mismatch_root_cause_and_consequences.md` answering the operator's two follow-up questions (Q1 root cause, Q2 consequences). iter-11 (correction-path application) is a **one-way door — requires explicit operator `correction` authorization event naming the chosen path (A / B / C / D), scope, and any supplementary mitigation before proceeding**. The loop will NOT advance to Wave 4 until operator resolves iter-11.

**Current research-log chain tip:** `061334bf9de08c4d228d5b2093fc2208db010cf83b9c742515d7b514c26fd6b2` (iter-10-supplemental, 2026-04-23).
