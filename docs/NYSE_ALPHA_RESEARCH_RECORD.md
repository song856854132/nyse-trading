...
D. Threshold pre-registration review — ~~only admissible if the plan's 0.02 in the text was the genuine original intent and 0.5 in gates.yaml was transcription error; must be logged as a `correction` event diff dated before re-screening.~~ **RESOLVED 2026-04-23 via correction path A (`docs/GOVERNANCE_LOG.md` GL-0010): divergence was family-level semantic redesign, not single-threshold typo; the in-force gate family is now canonical. See amended §357 paragraph above.**
E. Renegotiate Phase 3 exit target — "any factor passes G0-G5" rather than ensemble Sharpe 0.5–0.8. Calibrates the plan to what the data actually supports, not what the TWSE priors expected. **Remains open as future option; unrelated to path-A resolution of D.**

Path C is the recommended next action because (a) cheapest to run, (b) directly tests whether 5-day is the reason fundamentals are failing, (c) AP-6-compliant as a fresh forecast. (Path D was resolved on 2026-04-23 via correction path A — see GL-0010.)
...
**Wave-3 (C-gate-calibration) CLOSES with iter-11.** Wave 4 (multi-factor admission reform) unlocks. iter-0 bit-exactness preserved; hash chain preserved end-to-end via `scripts/verify_research_log.py`; TODO-11 and TODO-23 untouched.
