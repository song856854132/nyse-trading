...
| Gate family (`config/gates.yaml`) | **PROVISIONAL under active redesign** (see GL-0010 + GL-0012). sha256 `521b7571c330a5a1e87642eb9e5c0869ae8dc23cba3a1a175baf21a42f559af4` remains operationally in effect (unchanged since commit `339fa10` on 2026-04-18). GL-0010 (iter-11) canonicalized the in-force family; GL-0012 (iter-11-D, 2026-04-23 same day) reverses that canonicalization in response to adversarial governance review. Both the plan's pre-amendment family and the in-force family are now provisional pending v2 pre-registration in iter-13+ of Wave 4. No new admission decisions may be cited under either family until v2 pre-registration lands. |
| Phase 3 exit target (plan §Phase 3) | **Under renegotiation via PATH E** (see GL-0013). Original target OOS Sharpe 0.5–0.8 formally resolved as MISS under 0/6 factor admission state (see `docs/NYSE_ALPHA_RESEARCH_RECORD.md` §357). New target will be pre-registered before v2 gate family (GL-0012) is finalized, to avoid target-family double-fitting. |
...
| Last reviewed | 2026-04-23 (iter-11-D of 20-iter research loop; GL-0012 + GL-0013 land as path-D governance correction following `/codex` adversarial review; supersedes iter-11 path-A canonicalization from GL-0010) |
